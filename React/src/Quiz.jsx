...
export default function Quiz({setResult, setCompleted, isAdmin }) {
    let counter = 0;
    const { questions, setQuestions, answer, setAnswer, openNotification } = useQuestion()
    const [ order, setOrder] = useState({minOrder: 0,maxOrder: 3})
    const [ idModal, setModal ] = useState()
    useEffect(() => {
    (async () => {
        await axios.get('http://localhost:3000/admin/getquestions')
    ...
    useEffect(() => {
        ...
        Array.prototype.forEach.call(answer, (element,id) => element === questions[id].answer ? counter++ :null)
  ...
  return (
    <>
        <Form name='User' id='quiz-form' labelCol={{ span: 1 }} autoComplete="off" onFinish={Validate} >
            {questions?.map(( question,id ) => {
                if(id >= order.minOrder && id <= order.maxOrder)
                        return <Question key = {id} id = {id} isAdmin = {isAdmin} setOrder = {setOrder} order = {order} />
            })}
            <div className="quiz-control">
                ...
                >Prev</Button>
                {order.maxOrder < questions.length-1 && (<Button id='next' className='primary-btn right-btn' htmlType={isAdmin ?'button' :'submit'} onClick={Next}>Next</Button>)}
                {order.minOrder >= questions.length-4 && (
                    isAdmin ?<Button id='add-btn' className='primary-btn right-btn' onClick={() => setModal(true)} >Add Question</Button>
                    :<Button id='done' className='primary-btn right-btn' htmlType='submit' >Done</Button>
                )}
            </div>
